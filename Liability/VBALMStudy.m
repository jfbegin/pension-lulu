...
%            obj.TermEndAccLia = zeros(w + (ProjYear - EntryAge), nscen);
            ...
            CumCashflowMat = obj.CashflowBasic(1:(obj.ALMResult.w - EntryAge),:,:); 
            
            for i = 1:nscen
                TermEndAccLiaVec = zeros((obj.ALMResult.w - EntryAge), 1);
                for j = 1:(ProjYear)
                    TermEndAccLiaVec = (TermEndAccLiaVec - CumCashflowMat(:,j,i)).*(1+obj.ALMResult.ARMat(j,i)); 
                end
                TermEndAccLiaVec = TermEndAccLiaVec - CumCashflowMat(:,ProjYear+1,i);
                
                obj.TermEndAccLia(1:(obj.ALMResult.w-EntryAge), i) = TermEndAccLiaVec;
            end
            
            obj.TermEndAccLia = max(obj.TermEndAccLia, zeros(w + (ProjYear - EntryAge)+1, nscen));
            
%            obj.ResiValueAtEnd = zeros(w + (ProjYear - EntryAge), nscen);
            obj.ResiValueAtEnd = zeros(w + (ProjYear - EntryAge)+1, nscen);
